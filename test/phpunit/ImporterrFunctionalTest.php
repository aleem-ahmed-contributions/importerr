...
/**
 * \file    test/phpunit/ImporterrFunctionalTest.php
 * \ingroup importerr
 ...
 *
 * @package Testimporterr
 */
class ImporterrFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modImporterr')->click();
		} else {
			// Disable the module
			$this->byHref('modImporterr')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/importerr/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('importerr/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/importerr/admin/about.php');
		$this->authenticate();
		return $this->assertContains('importerr/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('importerrwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modImporterr_ImporterrTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modImporterr_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
